<template>
    <main>
                                ...
                                <td>
                                    <a href="#" class="btn btn-warning btn-sm"
                                       v-on:click.prevent="editDetailsCampaign(detail)" v-if="!detail.bus"><i
                                        ...
                                        Asignar Bus</a>
                                    <a href="#" class="btn btn-warning btn-sm"
                                       v-on:click.prevent="editDetailsCampaign(detail,'bus_change')" v-else><i
                                        class="fal fa-bus"></i>
                                        Cambiar Bus</a>
                                ...
                                <div class="buses_detalles">
                                    <div class="col-sm-6">
                                        ...
                                        <div>
                                            ...
                                            <span :class="(bus.tipo_contrato == 'Consumo') ? 'text-danger font-weight-bold': ''">{{bus.tipo_contrato}}</span>
                                        </div>
        ...
        <div class="button_save">
            <a href="#" class="btn btn-primary" v-on:click="saveCampaign">Guardar Asignación de Buses</a>
        </div>
    </main>
</template>
    ...
    export default {
        ...
        methods: {
            ...
            fixBusArray: function (bus) {
                this.bus_details = bus.fotografias.map((item) => item.image);
            },
            ...
            editDetailsCampaign: function (detail, action) {
                this.flag = true;
                if(action == 'bus_change'){
                    this.bus_change = true;
                }
                this.bus_details = [];
            ...
            addDetailCampaign: function (e) {
                ...
                let data = {
                    ...
                    previous_bus: this.previous_bus
                };

                axios.post(url, data).then((response => {
                    this.getCampaigns();
                    this.bus_change = false;
                    this.flag = false;
                    this.bus_details = [];
                    this.bus_fotografias = [];
                }));
            ...
            saveCampaign: function (e) {
                let empanada = this.details.map(function(values){
                    return values.bus;
                });

                let flag = true

                for(let i = 0; i <= empanada.length; i ++){
                    if (empanada[i] === null){
                        new Noty({
                            type: 'error',
                            layout: 'topRight',
                            text: 'Tienes que agregar los buses a las rutas.'
                        }).show();
                        flag = false
                        break;
                    }
