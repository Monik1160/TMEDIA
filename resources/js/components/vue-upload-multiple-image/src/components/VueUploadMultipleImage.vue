      ...
      <div class="image-bottom display-flex position-absolute full-width align-items-center justify-content-between"
        ...
        <div class="image-bottom-left display-flex align-items-center" v-if="showPrimary">
          <div class="display-flex align-items-center" v-show="imageDefault">
            <span class="image-primary display-flex align-items-center" v-if="approveDesign == 2">
              <svg class="image-icon-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><circle
                fill="#10BC83" cx="256" cy="256" r="256"></circle><path fill="#FFF"
                                                                        d="M216.7 350.9h-.1c-5.1 0-9.9-2.1-13.4-5.7l-74.2-76c-7.4-7.5-7.2-19.5.4-26.8 7.5-7.4 19.5-7.2 26.8.4L217 305l139.7-138.5c7.5-7.4 19.5-7.4 26.8.1s7.4 19.5-.1 26.8l-153.2 152c-3.7 3.5-8.5 5.5-13.5 5.5z"></path></svg>
              {{primaryText}}
            </span>
            <span class="image-primary display-flex align-items-center" v-else-if="approveDesign == 3">
              <svg class="image-icon-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><circle
                fill="red" cx="256" cy="256" r="256"></circle><path fill="#FFF"
                                                                        d="M216.7 350.9h-.1c-5.1 0-9.9-2.1-13.4-5.7l-74.2-76c-7.4-7.5-7.2-19.5.4-26.8 7.5-7.4 19.5-7.2 26.8.4L217 305l139.7-138.5c7.5-7.4 19.5-7.4 26.8.1s7.4 19.5-.1 26.8l-153.2 152c-3.7 3.5-8.5 5.5-13.5 5.5z"></path></svg>
              {{primaryText}}
            </span>
            <span class="image-primary display-flex align-items-center" v-else>
              <svg class="image-icon-primary" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><circle
                fill="grey" cx="256" cy="256" r="256"></circle><path fill="#FFF"
                                                                        d="M216.7 350.9h-.1c-5.1 0-9.9-2.1-13.4-5.7l-74.2-76c-7.4-7.5-7.2-19.5.4-26.8 7.5-7.4 19.5-7.2 26.8.4L217 305l139.7-138.5c7.5-7.4 19.5-7.4 26.8.1s7.4 19.5-.1 26.8l-153.2 152c-3.7 3.5-8.5 5.5-13.5 5.5z"></path></svg>
              {{primaryText}}
            </span>
            <label class="razon_label">Razón:</label>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      markIsPrimaryText: {
        type: String,
        default: 'Arte no enviado todavia a revisión'
      },
      dropText: {
        type: String,
    ...
    computed: {
      ...
      approveDesign() {
        let index = findIndex(this.images, {highlight: 1})
        if (parseInt(this.images[index].status) === 3){
          this.primaryText = 'Montaje Rechazado';
          this.popupText = this.images[index].decline_message;
        }else if (parseInt(this.images[index].status) === 2){
          this.primaryText = 'Montaje Aprovado'
          this.popupText = 'Montaje Aprovado'
        }else {
          this.primaryText = 'Montaje NO REVISADO'
          this.popupText = 'Montaje NO REVISADO'
        }
        return this.images[index].status
      },
    },
    methods: {
      onDrop(e) {
        ...
        reader.onload = (e) => {
          ...
          if (dataURI) {
            if (!this.images.length) {
              this.images.push({name: file.name, path: dataURI, highlight: 1, default: 1, status: 1})
              this.currentIndexImage = 0
            } else {
              this.images.push({name: file.name, path: dataURI, highlight: 0, default: 0, status: 1})
            }
